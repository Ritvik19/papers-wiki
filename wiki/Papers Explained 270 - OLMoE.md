...
This page ingests the source article into the wiki and connects it to [[Papers Explained Corpus]], [[Mixture of Experts]], [[Large Language Models]], [[Document AI]], [[Synthetic Data]], [[Reinforcement Learning Topic]], [[Supervised Fine-Tuning]].
...
- Pretrained on 5 trillion tokens, OLMoE-1B-7B outperforms all comparable open-source models and even rivals larger closed-source models.
- The models and dataset are available at [HuggingFace](https://huggingface.co/collections/allenai/olmoe-66cf678c047657a30c8cd3da).
- Recommended Reading [Papers Explained 98: OLMo](https://ritvik19.medium.com/papers-explained-98-olmo-fdc358326f9b)
- OLMoE is a decoder-only LM consisting of NL transformer layers. The feedforward network (FFN) in dense models like OLMo, is replaced with an MoE module consisting of NE smaller FFN modules called experts, of which a subset of k experts are activated for each...
- where r, called the router, is a learned linear layer mapping from the input logits to the chosen k experts. A softmax is applied to the router outputs to compute routing probabilities for all NE experts.
...
*Figure: Comparison of the architecture of dense LMs and MoE models.*
...
*Figure: Key MoE design choices and the setup for OLMoE-1B-7B.*
...
*Figure: Composition of the pretraining data for OLMoE-1B-7B.*
...
*Figure: Adaptation training data for OLMoE-1B-7B.*
...
*Figure: Evaluation of OLMoE-1B-7B and the current best OLMo models during pretraining.*
...
*Figure: OLMoE-1B-7B after pretraining versus larger MoEs and dense LMs.*
...
*Figure: OLMoE-1B-7B after adaptation versus other models.*
...
- [[Papers Explained: EMO]] - builds on the OLMoE-style MoE setting but changes pretraining to make expert subsets modular at inference.
