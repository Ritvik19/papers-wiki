...
This page ingests the source article into the wiki and connects it to [[Papers Explained Corpus]], [[Model Compression and Efficiency]], [[Large Language Models]].
...
- This study is based on Gemma 2 but the method can easily be applied to other model families. Additionally, this approach allows for unbalanced encoder-decoder models, where the decoder is significantly smaller than the encoder.
- For example, for summarization, deep understanding of the input text is often more important than the generation part, as it doesn’t need to generate any new information.
...
*Figure: Pretraining performance as a function of the number of pretrained tokens during the adaptation.*
...
*Figure: Main results on PT, IT, and SuperGLUE benchmarks.*
...
*Figure: Detailed results on different tasks for PT and RLHFed models.*
...
*Figure: Comparisons of decoder-only LLMs with adapted encoder-decoder models under inference flops.*
...
*Figure: Results for encoder-decoder models adapted with PrefixLM (Adaptation) and pretrained from scratch (Scratch).*
...
Encoder-Decoder Gemma: Improving the Quality-Efficiency Trade-Off via Adaptation [2504.06225](https://arxiv.org/abs/2504.06225)
