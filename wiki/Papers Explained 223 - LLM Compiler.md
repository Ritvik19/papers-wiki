...
- LLM Compiler models are initialized with the weights of Code Llama and then trained for 401 billion tokens on a compiler centric dataset composed mostly of compiler intermediate representations and assembly code generated by LLVM version 17.0.6.
- The idea is to generate from a finite set of unoptimized seed programs a large number of examples by applying randomly generated sequences of compiler optimizations to these programs. We then train the model to predict the code generated by the optimizations.
- The task specification involves generating the resulting code after applying optimization passes to unoptimized LLVM-IR (as emitted by the clang frontend) and a starting code size.
- The model is evaluated using two metrics for code size: the number of IR instructions and binary size (computed by summing the sizes of the .TEXT and .DATA sections of the IR or assembly after lowering to an object file).
...
*Figure: Training datasets used.*

*Figure: Composition of data used for initial IR and assembly pretraining.*
...
*Figure: Comparison of model performance when flag tuning 2,398 object files from MiBench.*
...
*Figure: Improvement over -Oz for each of the benchmarks in MiBench.*

*Figure: Accuracy of models at predicting code size before and after optimization.*
...
*Figure: Model performance at disassembling 2,015 assembly codes taken from MiBench.*
...
*Figure: Performance at next-token prediction and compiler emulation tasks.*
...
*Figure: Performance on Python programming tasks. pass@1 are computed with greedy decoding.*
...
Meta Large Language Model Compiler: Foundation Models of Compiler Optimization [2407.02524](https://arxiv.org/abs/2407.02524)
