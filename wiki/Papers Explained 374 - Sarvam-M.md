...
This page ingests the source article into the wiki and connects it to [[Papers Explained Corpus]], [[Large Language Models]], [[Reasoning Models]], [[Multilingual Models]], [[Code Models]], [[Synthetic Data]], [[Supervised Fine-Tuning]].
...
- Sarvam-M (M stands for Mistral) is a finetuned Mistral Small 24B. It significantly improves on the base model with large relative increases: +20% average improvement on Indian language benchmarks, +21.6% on math benchmarks, and +17.6% on programming...
- A large number of finetuning datasets, with completions from different models, are available on Hugging Face.
- Over 11.5 million prompts were collected from selected Hugging Face finetuning datasets. Using min-hash and fuzzy algorithms, this was reduced to about 7 million prompts.
- Recognizing the need for a more refined sampling strategy, each prompt was embedded using the gte-Qwen2–7B model and clustered into 100,000 clusters. Semantic deduplication within each cluster was performed with a cosine similarity threshold of 0.8.
...
*Figure: Difficulty Distribution*
...
*Figure: where p_i is the probability of digit i at the score token.*
...
[Sarvam — M: ‍Explorations in Post Training and Inferencing Optimizations for a Hybrid Indic LLM](https://www.sarvam.ai/blogs/sarvam-m)
