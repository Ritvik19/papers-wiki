...
This page ingests the source article into the wiki and connects it to [[Papers Explained Corpus]], [[Embedding and Retrieval]], [[Large Language Models]], [[Document AI]].
...
- Unlike traditional neural networks that utilise recurrence or convolution to process sequential data, transformers lack an inherent mechanism to recognize the order of input sequences.
- Positional encodings are added directly to the input embeddings at the initial layers of both the encoder and decoder stacks within a transformer model.
...
However, the original method proposed in [Self-Attention with Relative Position Representations](https://arxiv.org/abs/1803.02155) for calculating relative positional encodings was memory-intensive, scaling with the square of the sequence length (O(L²D)). To address this, [Music Transformer](https://arxiv.org/abs/1809.04281) introduced an efficient computation method through a skewing operation, focusing on optimizing the encoding process for the keys component and omitting the additional relative positional embedding for the value term.
...
*Figure: Implementation of Rotary Position Embedding(RoPE).*
...
- [[Papers Explained: Position Encodings in Transformers]] — Sinusoidal, Learned Absolute, and Shaw Relative encodings.
- [[Papers Explained: Rotary Position Embedding(RoPE)]] — Rotary Position Embedding mathematical foundation.
- [[Papers Explained: Attention with Linear Biases (ALiBi)]] — linear distance penalty attention biases.
- [[Papers Explained: No Position Encoding (NoPE)]] — implicit position encoding via causal attention masking.
- [[Papers Explained: Yet another RoPE extensioN method (YaRN)]] — NTK-by-parts context window extension.
