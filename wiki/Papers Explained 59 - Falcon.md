...
- Falcon shows that properly filtered and deduplicated web data alone can lead to powerful models; even significantly outperforming models from the state-of-the-art trained on The Pile.
...
*Figure: Subsequent stages of Macrodata Refinement remove nearly 90% of the documents originally in CommonCrawl.*
...
*Figure: To evaluate models trained on RefinedWeb and compare to the state-of-the-art, Four aggregates are built across 18 tasks on which to measure zero-shot performance.*
...
*Figure: Curation is not a silver bullet for zero-shot generalization: small-scale models trained on REFINEDWEB outperform models trained on web data (C4, OSCAR), and on curated corpora (The Pile)*
...
*Figure: Models trained on REFINEDWEB alone outperform models trained on curated corpora.*
...
*Figure: Although improvements from filtering are not systematic across datasets, deduplication brings a steady performance boost across the board.*
...
*Figure: Toxic content in RefinedWeb is distributed similarly to The Pile. Cumulative proportion of documents below a given toxicity score, as evaluated by the Pespective API.*

## Paper

The RefinedWeb Dataset for Falcon LLM: Outperforming Curated Corpora with Web Data, and Web Data Only [2306.01116](https://arxiv.org/abs/2306.01116)
