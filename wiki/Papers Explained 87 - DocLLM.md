...
DocLLM is a lightweight extension to traditional large language models (LLMs) for reasoning over visual documents, taking into account only textual semantics and spatial layout, avoiding expensive image encoders.

This page ingests the source article into the wiki and connects it to [[Papers Explained Corpus]], [[Large Language Models]], [[Document AI]], [[Reasoning Models]], [[Safety and Alignment]], [[Embedding and Retrieval]].
...
- Recommended Reading [Papers Explained 42: UDOP](https://medium.com/dair-ai/papers-explained-42-udop-719732358ab4)
- DocLLM is built upon the foundation of decoder-only transformer language models. In contrast to standard language models accepting only a sequence of text tokens as input, DocLLM integrates lightweight visual information by utilizing the spatial positions and...
...
- In classical transformers, using a learned embedding matrix based on the text vocabulary and a learned set of parameters for the token position in the sequence, the input tokens are first encoded into hidden vectors H.
- where Wq and Wk are projection matrices, and the superscript t indicates the text modality.
...
*Figure: DocLLM model architecture with disentangled spatial attention and infilling objective.*
...
*Figure: Pre-training dataset statistics.*
...
DocLLM: A layout-aware generative language model for multimodal document understanding [2401.00908](https://arxiv.org/abs/2401.00908)
