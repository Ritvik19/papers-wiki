...
DEtection TRansformer or DETR streamlines the detection pipeline, effectively removing the need for many hand-designed components like a non-maximum suppression procedure or anchor generation that explicitly encode the prior knowledge about the task using a set-based global loss that forces unique predictions via bipartite matching, and...
...
- DEtection TRansformer or DETR streamlines the detection pipeline, effectively removing the need for many hand-designed components like a non-maximum suppression procedure or anchor generation that explicitly encode the prior knowledge about the task using a...
- DETR infers a fixed-size set of N predictions, in a single pass. This causes difficulties in training to score the predicted objects wrt. to the ground truth.
...
*Figure: DETR directly predicts (in parallel) the final set of detections by combining a common CNN with a transformer architecture.*
...
End-to-End Object Detection with Transformers [2005.12872](https://arxiv.org/abs/2005.12872)
