...
mmBERT is an encoder-only language model pretrained on 3T tokens of multilingual text in over 1800 languages using an architecture inspired from ModernBERT. To build mmBERT, several novel elements are introduced, including an inverse mask ratio schedule and an inverse temperature sampling ratio.

This page ingests the source article into the wiki and connects it to [[Papers Explained Corpus]], [[Multilingual Models]], [[Large Language Models]], [[Embedding and Retrieval]], [[Code Models]], [[Evaluation and Benchmarks]].
...
- mmBERT is an encoder-only language model pretrained on 3T tokens of multilingual text in over 1800 languages using an architecture inspired from ModernBERT.
- Models and data are available at [HuggingFace](https://huggingface.co/collections/jhu-clsp/mmbert-a-modern-multilingual-encoder-68b725831d7c6e3acc435ed4/).
- An identical architecture to ModernBERT is used, but the Gemma 2 tokenizer handles multilingual input.
...
- The same three phase approach as ModernBERT is employed but with a novel inverse masking rate learning schedule. Instead of simply using a lower masking ratio at the end of training, the mask rate is progressively lowered at each stage.
...
*Figure: Common Configuration Parameters.*
...
*Figure: Inverse temperature sampling ratio throughout training from Fineweb2 data, from τ of 0.7 to 0.5 to 0.3.*
...
*Figure: Training data mixture across the various training stages.*
...
*Figure: GLUE (English) benchmark results.*

*Figure: XTREME benchmark results.*
...
*Figure: MTEB v2 English results.*
...
*Figure: MTEB v2 multilingual results.*
...
*Figure: Retrieval scores on the CoIR Benchmark.*
...
*Figure: Comparing EuroBERT and MMBERT on EuroBERT’s in-distribution languages on XNLI and PAWS-X.*
...
*Figure: Performance of models using different decay phases on two languages (Tigray and Faroese) only added during the decay phase.*
...
*Figure: Throughput efficiency for various sequence lengths and variable input length.*
...
mmBERT: A Modern Multilingual Encoder with Annealed Language Learning [2509.06888](https://arxiv.org/abs/2509.06888)
