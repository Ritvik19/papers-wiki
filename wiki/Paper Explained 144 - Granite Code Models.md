...
This page ingests the source article into the wiki and connects it to [[Papers Explained Corpus]], [[Code Models]], [[Reasoning Models]], [[Synthetic Data]].
...
- This paper introduces a series of decoder-only code models (3B, 8B, 20B, 34B) for code generative tasks, trained with code written in 116 programming languages, suitable for applications ranging from complex application modernization tasks to on-device...
...
- The project is available at [GitHub](https://github.com/ibm-granite/granite-code-models).
- The pretraining code data was sourced from a combination of publicly available datasets like Github Code Clean, StarCoderdata, and additional public code repositories and issues from GitHub.
...
*Figure: Model configurations for Granite Code models.*
...
*Figure: An overview of depth upscaling.*
...
*Figure: Summary of evaluation tasks.*

### HumanEvalSynthesize: Multilingual Code Generation in 6 Languages

*Figure: Pass@1 performance on HumanEvalSynthesize benchmark*
...
*Figure: Pass@1 results on MultiPL-E averaged over 50 samples for each problem.*
...
*Figure: Pass@1 on MBPP and MBPP+.*
...
*Figure: Mean pass@1 accuracy averaged over 40 samples on DS-1000.*
...
*Figure: Average exact match (EM) and edit similarity (ES) on RepoBench v1.1.*

*Figure: CrossCodeEval evaluation results.*
...
*Figure: Exact-match on FIM-task.*
...
*Figure: Pass@1 performance on HumanEvalExplain.*
...
*Figure: Pass@1 performance on HumanEvalFix.*
...
*Figure: Performance on HumanEvalPack.*

### Code Editing and Translation

*Figure: Pass@1 and ExcessCode performance on CanItEdit. Pass@1 assesses functional correctness, and ExcessCode assesses conciseness and precision of code edits.*
...
*Figure: Performance of different instruction-tuned models on CodeLingua.*
...
*Figure: Performance on the CRUXEval benchmark.*
...
*Figure: Performance on 4 chain-of-thought math tasks and 2 tool-aided math tasks.*
...
*Figure: Performance of Granite Code models on Berkeley Function-Calling Leaderboard.*

- The scaling of the model (from 3B to 34B parameters) significantly improved the function/tool calling capabilities.

*Figure: Granite-8B-Code vs CodeLlama-7B on Berkley Function-Calling Leaderboard.*
...
Granite Code Models: A Family of Open Foundation Models for Code Intelligence [2405.04324](https://arxiv.org/abs/2405.04324)
