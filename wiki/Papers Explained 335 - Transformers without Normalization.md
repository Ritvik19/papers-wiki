...
This work demonstrates that Transformers without normalization can achieve the same or better performance using a remarkably simple technique. Dynamic Tanh (DyT), an element-wise operation DyT(x) = tanh(αx), is introduced as a drop-in replacement for normalization layers in Transformers.
...
- Batch Normalization (BN): Primarily used in CNNs. Computes mean and variance across both the batch (B) and token (T) dimensions.
...
*Figure: Output vs. input of selected layer normalization (LN) layers.*
...
*Figure: Supervised classification accuracy on ImageNet-1K*
...
*Figure: Self-supervised learning accuracy on ImageNet-1K.*
...
*Figure: Image generation quality (FID, lower is better) on ImageNet.*
...
*Figure: Language models’ training loss and average performance with 15 zero-shot lm-eval tasks.*
...
*Figure: Speech pretraining validation loss on LibriSpeech.*
...
*Figure: DNA classification accuracy on GenomicBenchmarks.*
...
*Figure: Stability across varying α0 values, learning rates, and model sizes.*
...
*Figure: Optimal α0 for different LLaMA models.*
...
*Figure: Optimal α0 (attention / other) across model widths and depths in LLaMA training.*
...
Transformers without Normalization [2503.10622](https://arxiv.org/abs/2503.10622)
