...
This page ingests the source article into the wiki and connects it to [[Papers Explained Corpus]], [[Synthetic Data]], [[Embedding and Retrieval]], [[Safety and Alignment]], [[Large Language Models]], [[Reasoning Models]], [[Supervised Fine-Tuning]].
...
- The model is prompted with the chat template and sample. This is repeated a number of times to generate a set of synthetic data points.
- The synthetic data points are found to be from the same distribution as the alignment dataset.
- The pipeline functions as follows: the entire post-training set is embedded using an embedding model, constructing a vector search engine. A number of samples are then generated simply by prompting the model using a chosen prefix repeatedly.
- The observation that chat templates can be used to generate useful synthetic data has been pointed out already, where special prompts and a filtering pipeline are used to generate a dataset that can be used to post-train models using SFT.
...
*Figure: An overview of the data extraction process.*
...
*Figure: Model performance after SFT on the benchmarks considered by OLMo.*
...
*Figure: Count of training samples (prompt only) with likelihood above a certain threshold before and after RL.*
...
*Figure: RL training using the ORZ dataset and a dataset that was extracted.*

The model trained on the synthetic data extracted from the ‘Baseline’ achieves comparable performance on the benchmarks.

## Paper

Extracting alignment data in open models [2510.18554](https://arxiv.org/abs/2510.18554)
