...
Given a source text S, an LLM is prompted to identify a contiguous span t of important reasoning steps. This span t is used to construct a masked context Smask by replacing t in S with a special token [MASK]. Treating t as the ground-truth answer, the LLM then generates a set of diverse distractors D= {d1, d2, . . .

This page ingests the source article into the wiki and connects it to [[Papers Explained Corpus]], [[Large Language Models]], [[Reasoning Models]], [[Reinforcement Learning Topic]], [[Synthetic Data]], [[Verifier-Bounded Learning]], [[Reinforcement Learning]], [[Model Distillation]].
...
- Given a source text S, an LLM is prompted to identify a contiguous span t of important reasoning steps. This span t is used to construct a masked context Smask by replacing t in S with a special token [MASK].
- If the source text S is noisy, such as cybersecurity-related scrapes from FineWeb, the LLM is first prompted to extract or summarize S into a coherent, educationally valuable S′. Smask and D are then constructed based on S′.
...
- AoPS-Instruct: Extracted around 600k question-answer pairs from the Art of Problem Solving (AoPS) forum, featuring Olympiad-level math problems and community-driven solutions.
...
*Figure: The Golden Goose pipeline.*
...
*Figure: Comparison of continued RL training on ProRL-1.5B-v2 using the original ProRL data, adding GooseReason-0.7M, or using RLVE.*
...
*Figure: Comparison of RL training from scratch on Qwen-4B-Instruct under a fixed compute budget with ProRL data only versus joint training with GooseReason-0.7M.*
...
Golden Goose: A Simple Trick to Synthesize Unlimited RLVR Tasks from Unverifiable Internet Text [2601.22975](https://arxiv.org/abs/2601.22975)
