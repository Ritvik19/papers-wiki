...
DeepSeek-OCR 2 inherits the overall architecture of DeepSeek-OCR, which consists of an encoder and a decoder. The encoder discretizes images into visual tokens, while the decoder generates outputs conditioned on these visual tokens and text prompts.

This page ingests the source article into the wiki and connects it to [[Papers Explained Corpus]], [[Document AI]], [[Large Language Models]], [[Reasoning Models]], [[Computer Vision]], [[Vision Language Models]].
...
- This architecture is instantiated using Qwen2–0.5B, whose 500M parameters are comparable to CLIP ViT (300M) without introducing excessive computational overhead.
...
- where I ∈R𝐻×𝑊×3 is the input image, E is the vision tokenizer mapping images to 𝑚 visual tokens V ∈R𝑚×𝑑, Q0 ∈R𝑛×𝑑 are learnable causal query embeddings, ⊕ denotes sequence concatenation, T𝐿 represents an 𝐿-layer Transformer with masked attention, M ∈{0...
...
*Figure: Token count calculation in DeepEncoder V2.*
...
*Figure: Attention mask architecture of DeepEncoder V2.*
...
*Figure: Comprehensive evaluation of document reading on OmniDocBench v1.5.*
...
*Figure: Edit Distances for different categories of document-elements in OmniDocBench v1.5.*
...
*Figure: Detailed comparison between DeepSeek-OCR 2 and DeepSeek-OCR across 9 document types.*
...
*Figure: Production performance comparison between DeepSeek-OCR and DeepSeek-OCR 2.*
...
DeepSeek-OCR 2: Visual Causal Flow [2601.20552](https://arxiv.org/abs/2601.20552)
