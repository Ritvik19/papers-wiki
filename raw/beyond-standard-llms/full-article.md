Source URL: https://magazine.sebastianraschka.com/p/beyond-standard-llms
Title: Beyond Standard LLMs - by Sebastian Raschka, PhD
...
Transformer-based LLMs based on the classic _Attention Is All You Need_ architecture are still state-of-the-art across text and code. If we just consider some of the highlights from late 2024 to today, notable models include
...
Now, what is Gated DeltaNet? Gated DeltaNet (short for _Gated Delta Network_) is Qwen3-Next’s linear-attention layer, which is intended as an alternative to standard softmax attention. It was adopted from the Gated Delta Networks: Improving Mamba2 with Delta Rule paper as mentioned earlier.
...
_**Figure 18:** Conceptual overview of a world model system. The agent interacts with the environment by observing its current state(t) and taking action(t) to achieve a givenobjective. In parallel, the agent learns an internal world mode**l**, which serves as a mental simulation of the environment, which allows it to predict outcomes and plan actions before executing them in the real world._
