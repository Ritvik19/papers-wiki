...
This work proposes a training method for equipping existing LLMs with such thinking abilities for general instruction following without use…
...
Evaluation
Benchmark win rates (%) for AlpacaEval (length-controlled (LC)) and Arena-Hard.Training iterations on AlpacaEval (left) and Arena-Hard (right), comparing our TPO
method to the direct baseline starting from the seed (iteration 0) model.Breakdown of AlpacaEval results for different judge models, training instruction sets and Thought Prompts.
...
By Ritvik Rastogi on December 18, 2024.
