...
2. **Decontamination**: We detect and remove all benchmark items on which we evaluate, using exact string match on both the solutions and prompts.
3. **Docstring Quality Filtering**: We use StarCoder2-15B as a judge to remove functions with poor documentation, prompting the base model with 7 few-shot examples, requiring it to respond with either "Yes" or "No" for retaining the item.
4. **Near-Deduplication**: We use MinHash and locality-sensitive hashing with a Jaccard similarity threshold of 0.5 to filter duplicate seed functions, the same process applied to StarCoder's training data.

This filtering pipeline results in a dataset of 250k Python functions filtered from 5M functions with docstrings, a process highly inspired by the data collection pipeline used in MultiPL-T.

### Self-OSS-Instruct

After collecting the seed functions, we use Self-OSS-Instruct to generate diverse instructions. We employ in-context learning to let the base StarCoder2-15B self-generate instructions from the given seed code snippets, using 16 carefully designed few-shot examples, each formatted as (snippet, concepts, instruction). The instruction generation procedure is divided into two steps:

1. **Concepts extraction**: For each seed function, StarCoder2-15B is prompted to produce a list of code concepts present within the function (e.g., pattern matching, data type conversion).
...
- bigcode/starcoder2-15b-instruct-v0.1 (16B, Text Generation)

## Datasets mentioned in this article

- bigcode/self-oss-instruct-sc2-exec-filter-50k
- bigcode/the-stack-v2 (seed source, referenced)
- bigcode/ds-1000 (evaluation benchmark)
