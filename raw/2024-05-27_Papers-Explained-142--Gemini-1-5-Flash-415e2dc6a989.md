...
Time per output character (ms) of various APIs for English, Japanese, Chinese, and French responses, given inputs of 10,000 characters.
...
Cumulative average negative log-likelihood (NLL) as a function of token position in long documents and code data.
...
Retrieval performance of the “multiple needles-in-haystack” task, which requires retrieving 100 unique needles in a single turn.Cumulative average string similarity score as a function of context length over 2000 instances of the MRCR task.
...
Quantitative results for Kalamang→English translation on MTOBQuantitative results for English→Kalamang translation on MTOB.
...
Character Error Rate (CER, ↓) for Gemini 1.5 Pro learning Kalamang ASR in context.Character Error Rate (CER, ↓) for Gemini 1.5 Flash learning Kalamang ASR in context.
...
In-context learning scaling results (chrF↑) on low-resource translation.
...
Answer quality based on side-by-side auto-rater (Gemini 1.5 Pro), rankings and scores (𝑒 𝛽 ) computed via the Bradley-Terry Model.
...
Comparison between 1H-VideoQA and EgoSchema.Comparison between GPT-4V and Gemini 1.5 models on 1H-VideoQA.
...
PDDL Planning and Natural Language Planning with few-shots.
...
Performance of models on unstructured data analytics tasks.
...
Performance of different models on Expertise QA. 95% confidence intervals via bootstrapping.
Gemini 1.5 models significantly outperform 1.0 Pro on Expertise QA.
Evaluation results of Gemini 1.5 Pro, 1.5 Flash and Gemini 1.0 models on Domain-specific methodical tasks from Dolom ites benchmark. (Metric: LM-judge win rate).
Gemini 1.5 Pro achieves the highest win-rate of 55.3% in Domain-Specific Long-Form Methodical Tasks.
STEM QA with Context: proportions [%] of accurate and inaccurate sentences in Gemini 1.0 and 1.5 model responses on the Qasper dataset. 95% C.I. obtained by bootstrap.
...
Comparison of Gemini 1.5 Pro with Gemini 1.0 Pro and Ultra on video understanding benchmarks.
...
A sampling of evaluation results comparing Gemini 1.5 Flash-8B and Gemini 1.5 Flash models on a few standard evaluations spanning capabilities and modalities. U
...
By Ritvik Rastogi on May 27, 2024.
