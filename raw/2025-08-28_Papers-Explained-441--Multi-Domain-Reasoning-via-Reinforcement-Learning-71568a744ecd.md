...
This study investigates multi-domain reasoning within the RLVR framework, addressing the gap in understanding the interplay among different…
...
Data:
Datasets for multi-domain training.
Rewards:
...
Math Domain
Model performance (%) after training in the math domain.
...
Model performance (%) after training in the code domain.
...
Puzzle Domain
Model performance (%) after training in the puzzle domain.
...
Performance (%) of the RL model with ∆ compared to Base.
...
Performance comparison of tripledomain and optimal dual-domain data.
...
<|im start |>user
{question}<|im end|>
<|im start|> assistant{question}Model performance under different templates.
...
The average test performance of base and instruct models on different templates.
...
Model performance on the KK dataset with different curriculum settings.
...
KK-impact of reward configurations.LPB-impact of reward configurations (base model shown with dashed lines).
...
The effect of different training language.
...
Can One Domain Help Others? A Data-Centric Study on Multi-Domain Reasoning via Reinforcement Learning 2507.17512
...
By Ritvik Rastogi on August 28, 2025.
