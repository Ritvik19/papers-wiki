...
DiffusionGemma reverses this inefficiency. Instead of predicting words sequentially, it drafts an entire 256-token paragraph simultaneously. By giving the computer's processor a larger chunk of work at once, DiffusionGemma utilizes your hardware to its full potential.
...
- Download the weights on Hugging Face (Apache 2.0).
- Integrate via DiffusionGemma developer guide, Visual Guide to DiffusionGemma.
- Serve with MLX, vLLM, Hugging Face Transformers; fine-tune with Hackable Diffusion, Unsloth, NVIDIA NeMo; llama.cpp support arriving soon.
- Optimized for NVIDIA consumer (RTX 4090/5090) and enterprise (Hopper/Blackwell NVFP4) hardware.
- Deploy via Gemini Enterprise Agent Platform Model Garden or NVIDIA NIM.

Note: Unified-memory architectures like Apple Silicon may not see the same acceleration over autoregressive Gemma 4 because they are often memory-bandwidth-bound rather than compute-bound during inference.
