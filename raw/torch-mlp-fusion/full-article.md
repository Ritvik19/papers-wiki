...
2. Profiling in PyTorch (Part 2): From nn.Linear to a Fused MLP _(current)_
...
Where `x` is the input, `w` is the weight and `b` is the bias. Let's run 02\_linear.py and check the profile.
...
| ---------- | ------------ | --------------------------------------------------------- | ---------------------------------- |
| gate\_proj | aten::linear | ampere\_bf16\_s16816gemm\_bf16\_128x128\_...              | occupancy query + cudaLaunchKernel |
| up\_proj   | aten::linear | ampere\_bf16\_s16816gemm\_bf16\_128x128\_...              | occupancy query + cudaLaunchKernel |
| gelu       | aten::gelu   | vectorized\_elementwise\_kernel<4, GeluCUDAKernelImpl...> | cudaLaunchKernel                   |
| h \* u     | aten::mul    | vectorized\_elementwise\_kernel<4, ...MulFunctor...>      | cudaLaunchKernel                   |
| down\_proj | aten::linear | ampere\_bf16\_s16816gemm\_bf16\_128x256\_...              | occupancy query + cudaLaunchKernel |
...
| ---------- | ---------------------------- | ------------- | ------------------------- | -------- |
| gate\_proj | \[8192,768\] x \[768,3072\]  | 8192·768·3072 | …128x128…stages\_32x5\_tn | 0.19ms   |
| up\_proj   | \[8192,768\] x \[768,3072\]  | 8192·768·3072 | …128x128…stages\_32x5\_tn | 0.19ms   |
| down\_proj | \[8192,3072\] x \[3072,768\] | 8192·3072·768 | …128x256…stages\_64x3\_tn | 0.17ms   |
...
1. **The fusion is baked in.** The LigerGEGLUMLP forward is `down_proj(LigerGELUMulFunction.apply(gate_proj(x), up_proj(x)))`. The LigerGELUMulFunction runs a single Triton kernel, \_geglu\_tanh\_forward\_kernel, that computes `gelu(gate) * up` in one pass. This is exactly what we saw from `torch.compile`, where the intermediate never makes a round-trip through HBM. We get it here **without the compiler**, as shown in Figures 13 and 14 (no Dynamo guards, no compile latency, no recompilation risk).
2. **The launch parameters were chosen for the hardware.** The kernel does not guess its block size at random. Liger's calculate\_settings picks them from the column count.
...
ariG23498/profiling-pytorch Updated 4 days ago •  161 •  2 

More Articles from our Blog

torchprofileCUDA Profiling in PyTorch (Part 1): A Beginner's Guide to torch.profiler +1  139 May 29, 2026 

torchprofileflash Profiling in PyTorch (Part 3): Attention is all you profile  10 July 10, 2026 
...
thanks bro, it's really help, looking forward your next blog!
...
Thanks a bunch for this blog post series! A good habit: _Reason first, then look_ 👍.

There are some parts I have been thinking about:

1. When the input data violates the strides precomputed by the compiler, will it throw an error or re-compile?
2. It is said that "Each GEMM is still the same bias-free cuBLAS kernel as a standalone linear", but the current one has never had a bias in the first place. The previous one was bias-free, with `aten::addmm`, the current one is also bias-free but with `aten::mm`.

WDYT? I am looking forward to the third blog post!
...
1. I think it would recompile, as has been the case with `torch.compile`. There is an official article on how to deal with recompilation from the torch team.
2. Great catch! That is correct.
...
Thanks for the intuition-led blog. Really appreciate it! :)
...
I am incredibly excited for the third blog post.
