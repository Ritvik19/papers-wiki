...
Nemotron-H is a family of 8B and 56B/47B hybrid Mamba-Transformer models designed to reduce inference cost for a given accuracy level. This…
...
Model Architecture
Nemotron-H-8B/56B model architectures.Summary of the Nemotron-H hybrid Mamba-Transformer architectures.
...
Data mixtures for each phase of Nemotron-H pre-training.
...
MiniPuzzle’s optimization process.
...
Comparison of benchmark scores for pruned candidates before and after lightweight distillation.
Results
MMLU-Pro accuracy versus per-GPU inference throughput for Nemotron-H-56B/47B-Base compared to existing similarly-sized Transformer models.Accuracy of Nemotron-H-56B-Base (and its distilled version Nemotron-H-47B-Base) versus existing SoTA models.Accuracy of Nemotron-H-8B-Base versus existing SoTA models and a Transformer (Nemotron-T-8B-Base) trained on exactly the same data.
...
Evaluation
Evaluation of Nemotron-H-8B-VLM on vision-language benchmarks.
Nemotron-H-8B-VLM proves to be a strong LLM backbone for developing best-in-class VLMs.
Evaluation ofNemotron-H-56B-VLM on a range of vision-language benchmarks, compared to a VLM built using Qwen2.5–72B-Instruct with the same training recipe.
...
Comparison of benchmarks after post-training.
...
Comparison of experimental aligned models on key alignment benchmarks.
...
By Ritvik Rastogi on September 15, 2025.
