...
1. Profiling in PyTorch (Part 1): A Beginner's Guide to torch.profiler _(current)_
...
> Here is the entire script that we use for the post: 01\_matmul\_add.py. We recommend opening this script in a separate tab and walk through the code step by step. We use the `NVIDIA A100-SXM4-80GB` GPU to run the scripts. It is really easy to setup a GPU on the Hugging Face infrastructure and experiment with the scripts using the Dev Mode with Spaces. One could also run the scripts with the Hugging Face Jobs pipeline.
...
Let's see the two in action with our first execution. (Here is the entire 01\_matmul\_add.py script)
...
We now move into visualising the dispatch chain, which lives inside the `.json` artifacts. You can upload them to Perfetto UI and see the traces, or you can use `uvx trace-util -f traces -b <hf_uname>/traces` to generate the Perfetto links directly.
...
| Step | matmul\_add start | aten::matmul start | gap        |
| ---- | ----------------- | ------------------ | ---------- |
...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Figure 6: The \~228 µs dead window between record\_function("matmul\_add") and aten::matmul                                                                                                                             |
...
> PyTorch provides the torch.addmm function that does what we did in two steps, that is multiply and add. We encourage the reader to look at the traces of this function and comment your observations in the comments below!
...
| Big gap between record\_function("...") start and the first aten::\* inside it | Same cold-start tax, just zoomed in. The annotation entered, but the dispatch hadn't happened yet.                                                                           |
...
| A kernel named like ampere\_bf16\_s16816gemm\_... | The actual cuBLAS GPU work for a matmul. The kernel name is typically the same in eager and compiled mode for the same shapes/dtypes.       |
...
| What you see                                                                          | What it usually means                                                                                                       |
| ------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| ProfileStep#N → <record\_function name> → aten::\* → aten::mm / aten::bmm / aten::add | The canonical nested call hierarchy. Self time excludes children; Total time includes them.                                 |
...
| \## Call CompiledFxGraph <hash> with the same hash across steps       | Cache hits on the generated code. The generated source lives under /tmp/torchinductor\_<user>/fxgraph/<hash>.   |
...
torchprofileCUDA Profiling in PyTorch (Part 2): From nn.Linear to a Fused MLP +1  53 June 11, 2026 
...
Thanks a bunch for this blog post series -really much needed! Especially the Trace reading cheatsheet sections are very beneficial.

There are some parts I have been thinking about:

1. Why are we setting `bias` to a 2D matrix? Isn't it supposed to be 1D? When I set it to a 1D vector, `cudaMemcpyAsync` is gone in the "Do the CUDA launches go down by half?" section, and there is only one kernel for `matmul_add`: `ampere_bf16_s16816gemm_bf16_128x256_ldg8_relu_f2f_stages_64x3_nn`.
2. IMO, we don't need `torch.cuda.synchronize()` in the end, since doesn't the context manager's `__exit__()` do it already?
